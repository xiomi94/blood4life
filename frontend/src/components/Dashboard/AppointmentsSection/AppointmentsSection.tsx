...
import ViewDonorModal from '../../Modals/ViewDonorModal/ViewDonorModal';
...
const AppointmentsSection: React.FC<AppointmentsSectionProps> = ({ appointments }) => {
    ...
    const handleViewNext = async () => {
        ...
        try {
            ...
            console.error(error);
            alert(t('dashboard.appointments.noNextAppointment') || 'No se encontró una próxima cita');
        } finally {
    ...
    return (
        <section className="mb-8 relative">
            <div className="flex justify-between items-center mb-4">
                ...
                >
                    {loadingNext ? 'Cargando...' : 'Ver Próxima Cita'}
                </button>
            </div>
                                    ...
                                    {appointment.donorCompletedAppointments !== undefined && (
                                        <div className="pt-2 border-t border-gray-100 dark:border-gray-700 mt-2">
                                            <p className="text-xs text-gray-500">
                                                Horas/Donaciones: <span className="font-semibold text-gray-700 dark:text-gray-300">{appointment.donorCompletedAppointments}</span>
                                            </p>
                                        </div>
                                    )}
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full p-6 relative animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="space-y-4">
                            <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-100 dark:border-red-800">
                                <p className="text-sm text-gray-500 dark:text-gray-400 uppercase tracking-wide">Fecha y Hora</p>
                                <p className="text-2xl font-bold text-red-600 dark:text-red-400">
                                    {nextAppointment.dateAppointment} <span className="text-lg text-gray-600 dark:text-gray-300">at {nextAppointment.hourAppointment}</span>
                                </p>
                            </div>

                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">Donante</p>
                            ...
                            {nextAppointment.donorCompletedAppointments !== undefined && (
                                <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                    <p className="text-sm text-gray-600 dark:text-gray-300">
                                        Historial: <span className="font-bold">{nextAppointment.donorCompletedAppointments}</span> donaciones completadas
                                    </p>
                                </div>
                            )}
                        </div>
