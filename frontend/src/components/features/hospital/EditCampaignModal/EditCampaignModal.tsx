...
const EditCampaignModal: React.FC<EditCampaignModalProps> = ({ isOpen, campaign, onClose, onSuccess }) => {
    ...
    // Función para marcar/desmarcar tipos de sangre
    const handleBloodTypeToggle = (bloodType: string) => {
        ...
        if (currentTypes.includes(bloodType)) {
            // Si ya está, lo quitamos (filtramos todos menos ese)
            setFormData({
                ...formData,
        ...
        // Si había error en este campo, lo limpiamos
        if (errors.requiredBloodTypes) {
            setErrors({ ...errors, requiredBloodTypes: '' });
        }
    };

    // Validamos que todos los datos estén correctos antes de enviar
    const validateForm = (): boolean => {
        ...
        // Validar lógica de fechas (fin debe ser después de inicio)
        ...
        setErrors(newErrors);
        // Si no hay keys en el objeto de errores, es que es válido
